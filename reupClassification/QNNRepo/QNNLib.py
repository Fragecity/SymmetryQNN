...
def enc_func_2d_even(x, parameters, num_qubits):
    """Encode classical data into quantum states."""
    for i in range(num_qubits):
        if i % 2 == 0:
            qml.RX(x[0]*np.pi*2, wires=i)
        else:
            qml.RY(x[1]*np.pi*2, wires=i)
...
def cir1_multi(parameters, num_qubits):
    for i in range(num_qubits):
        qml.RX(parameters[2*i], wires=i)
        qml.RZ(parameters[2*i+1], wires=i)
