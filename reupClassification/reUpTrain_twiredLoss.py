...
from reupClassification.Cleaners import optimizerSelector, dsLoader, genDecisionArea, optmizedParam, sgwScheduler
from reupClassification.QNNRepo.QNNLib import enc_func, cir1, cir2, cir2wl, cir_testSG, cir2wl_cz, enc_func_2d_even
...
# data
DS_TYPE = 'small_100'
BATCH_SIZE = 10  # 32(small) 6(Small30) 3(verySmall) small60/small60_xhalf(12)
#* model
...
if NUM_QUBITS == 4:
    ...
    ob2 = tensorOfListOps2(Id, Z, Id, Id, Id, Id)
    symGroup = [tensorOfListOps2(Id, Id, Id, Id, Id, Id), np.kron(SWAP, np.kron(SWAP, SWAP))]
...
for i in range(NUM_EPOCH):
    ...
    for j in range(0, NUM_BATCH):
        ...
        params_taped = opt.step(partial(cost, qnn_ob1=qnn_ob1, qnn_ob2=qnn_ob2, data_batch=batch), params_taped)
        cst = cost(params_taped, qnn_ob1, qnn_ob2, train_ds)
        acc = accuracy(qnn_ob1, qnn_ob2, params_taped, train_ds)
        cst_List.append(cst)
        acc_List.append(acc)
        if sgOn:
            paramsG_taped = optG.step(partial(costTwiredLoss, qnn_ob1=qnn_ob1, qnn_ob2=qnn_ob2, symmetry_ob1=symmetry_ob1,
                                              symmetry_ob2=symmetry_ob2, data_batch=batch), paramsG_taped)
            # print(params_taped)
            cstG = costTwiredLoss(paramsG_taped, qnn_ob1, qnn_ob2, symmetry_ob1, symmetry_ob2, train_ds)
            accG = accuracy(qnn_ob1, qnn_ob2, paramsG_taped, train_ds)
...
# * print final result
acc_train, acc_test = accuracy(qnn_ob1, qnn_ob2, params_taped, train_ds), accuracy(qnn_ob1, qnn_ob2, params_taped, test_ds)
accG_train, accG_test = accuracy(qnn_ob1, qnn_ob2, paramsG_taped, train_ds), accuracy(qnn_ob1, qnn_ob2, paramsG_taped, test_ds)
...
# * print best result
bestTrainAccTest, bestTrainAccGTest = accuracy(qnn_ob1, qnn_ob2, bestTrainParams, test_ds), accuracy(qnn_ob1, qnn_ob2, bestTrainParamsG, test_ds)
...
genDecisionArea(qnn_ob1, qnn_ob2, bestTrainParams, num_sample=50,  title="loss", ax=axs[0])
genDecisionArea(qnn_ob1, qnn_ob2, bestTrainParamsG, num_sample=50,  title="lossG", ax=axs[1])
...
fig, axs = plt.subplots(1, 3, figsize=(16, 5))  # figsize can be adjusted as per your preference
