# #! file name means: Qiskit 2-qubits system: Data_Network_Symmetry Setting 1
...
train_ds = pured_data[:CUT]
val_ds = pured_data[CUT:]

#* PQC and its ini with data
qc = QuantumCircuit(NUM_PART*6,1) #? why one classical bit?
...
qnn_s = QNN(qc)
qnn_t = QNN(qc)
qnn_s.encode('initialize', train_ds)
qnn_t.encode('initialize', val_ds)
...
sym_set.append(swap)
sym_set = [tensor(IDd, op, IDd) for op in sym_set]
...
#* Save
import pickle
with open('./data/test_2b_dns_1.pkl', 'wb') as f:
